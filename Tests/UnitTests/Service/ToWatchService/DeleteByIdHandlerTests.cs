...
namespace Tests.Service.ToWatchService
{
    ...
    public class DeleteByIdHandlerTests
    {
        ...
        public async Task Initialize()
        {
            ...
            handler = new DeleteByIdHandler(userDetailsRepository);
            await SeedData();
        }
        ...
        {
            await handler.Handle(new DeleteByIdCommand(gameId, userId), CancellationToken.None);
        ...
        public async Task TestRemove_ShouldThrowDomainException_WhenMediaIsNotInWatchList()
        {
            ...
            await Assert.ThrowsExactlyAsync<DomainException>(async()=>await handler.Handle(new DeleteByIdCommand(nonExistentMediaId, userId), CancellationToken.None));
            var result = appDbContext.UsersDetails.Include(u => u.UserInteractions).FirstOrDefault(u => u.Id == userId);
            Assert.IsNotNull(result);
