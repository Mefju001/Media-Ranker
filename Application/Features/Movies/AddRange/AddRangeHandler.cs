...
{
    public class AddRangeHandler : IRequestHandler<AddRangeCommand, List<MovieResponse>>
    {
        private readonly IGenreHelperService genreHelperService;
        private readonly IDirectorHelperService directorHelperService;
        private readonly IMediaRepository<Domain.Aggregate.Movie> mediaRepository;
        private readonly IMediator mediator;
        //maybe add better response with info about which games were added and which not, and why.
        public AddRangeHandler(IMediaRepository<Domain.Aggregate.Movie> mediaRepository, IGenreHelperService genreHelperService, IMediator mediator, IDirectorHelperService directorHelperService)
        {
        ...
        public async Task<List<MovieResponse>> Handle(AddRangeCommand requests, CancellationToken cancellationToken)
        {
            ...
            var movies = requests.movies.Select(movieReq =>
            {
                ...
                return Domain.Aggregate.Movie.Create(movieReq.Title,
