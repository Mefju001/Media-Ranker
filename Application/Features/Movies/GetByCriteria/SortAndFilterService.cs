...
namespace Application.Features.Movies.GetMoviesByCriteria
{
    public class SortAndFilterService : ISortAndFilterService
    {
        ...
        public async Task<List<MovieResponse>> Handler(GetByCriteriaQuery request, CancellationToken cancellationToken)
        {
            var query = appDbContext.Set<Movie>().AsNoTrackingWithIdentityResolution().AsSplitQuery();
            query = ApplyFilters(query, request);
            query = ApplySorting(query, request);
            return await query
                .Join(appDbContext.Set<Domain.Aggregate.Genre>(), mov => mov.GenreId, gen => gen.Id, (mov, gen) => new { mov, gen })
                .Join(appDbContext.Set<Domain.Aggregate.Director>(), mg => mg.mov.DirectorId, dir => dir.Id, (mg, dir) => new { mg.mov, mg.gen, dir })
                .Select(m => MovieMapper.ToMovieResponse(m.mov, m.gen, m.dir))
                .ToListAsync(cancellationToken);
        }
        private IQueryable<Movie> ApplyFilters(IQueryable<Movie> query, GetByCriteriaQuery request)
        {
        ...
        private IQueryable<Movie> ApplySorting(IQueryable<Movie> query, GetByCriteriaQuery request)
        {
